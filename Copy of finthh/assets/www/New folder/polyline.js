...
function createMap() {
  ...
  //get location from phonegap api
...
function jumpToPoint() {
  ...
  if (pointList.selectedIndex >= 0) {
	...
	copyArea.value="";
	for (var a=0; a<pointList.options.length; a++)
		{
		if (pointList.options[a].text) {
				s=pointList.options[a].text;
				s=s.replace(/\(/g,"");
				s=s.replace(/\) Level: \d*/,"");
				copyArea.value=copyArea.value+s+"\n";}
			    }
	}
